...
async function fetchAthleteData(
  ...
  return {
    ...
    // Merge rows per date: dailyLoad comes from WORKOUT rows; acwr comes
    // from the ACWR_SUMMARY row, whose dailyLoad duplicates the workout
    // rows and must not be added.
    trainingLoads: (() => {
