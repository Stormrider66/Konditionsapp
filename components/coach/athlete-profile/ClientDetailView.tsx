// components/coach/athlete-profile/ClientDetailView.tsx
//
// Client view for the coach athlete profile. All six datasets are loaded in
// parallel on the server (see the page.tsx orchestrator) and passed in as
// `initial` — no fetch-on-mount waterfall. The remaining fetchers exist only
// to refresh after mutations inside the tabs.
'use client'
...
export interface InitialProfileData {
  client: ClientWithTests | null
  error: string | null
  programs: ProgramSummary[]
  sportProfile: SportProfileSummary | null
  recentTests: RecentTestEntry[]
  recentTestCounts: RecentTestCounts
  athletePlans: AthletePlanSummary[]
...
interface ClientDetailViewProps {
  id: string
  businessSlug: string
  initial: InitialProfileData
}

export function ClientDetailView({ id, businessSlug, initial }: ClientDetailViewProps) {
  ...
  const [client, setClient] = useState<ClientWithTests | null>(initial.client)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(initial.error)
  const [programs] = useState<ProgramSummary[]>(initial.programs)
  const [programsLoading] = useState(false)
  const [sportProfile, setSportProfile] = useState<SportProfileSummary | null>(initial.sportProfile)
  const [sportProfileLoading] = useState(false)
  const [recentTests, setRecentTests] = useState<RecentTestEntry[]>(initial.recentTests)
  const [recentTestCounts, setRecentTestCounts] = useState<RecentTestCounts>(initial.recentTestCounts)
  const [athletePlans, setAthletePlans] = useState<AthletePlanSummary[]>(initial.athletePlans)
  const [athletePlansLoading] = useState(false)
  const [teamPlan] = useState<TeamPlanContext | null>(initial.teamPlan)
