  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Skaderapporter</h1>
      ...
      <div className="mb-6 flex flex-col gap-3 rounded-2xl border border-white/10 bg-slate-900/50 p-4 md:flex-row">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="Sök spelare, kroppsdel eller beskrivning..."
        ...
        <Select value={status} onValueChange={setStatus}>
          ...
          <SelectContent className="border-white/10 bg-slate-900">
            <SelectItem value="PENDING_REVIEW" className="text-slate-200">Nya rapporter</SelectItem>
            <SelectItem value="REVIEWED" className="text-slate-200">Bedömda</SelectItem>
            <SelectItem value="IN_TREATMENT" className="text-slate-200">I behandling</SelectItem>
            <SelectItem value="RESOLVED" className="text-slate-200">Lösta</SelectItem>
            <SelectItem value="all" className="text-slate-200">Alla statusar</SelectItem>
          </SelectContent>
        </Select>
        <Select value={urgency} onValueChange={setUrgency}>
          ...
          <SelectContent className="border-white/10 bg-slate-900">
            <SelectItem value="all" className="text-slate-200">Alla nivåer</SelectItem>
            <SelectItem value="EMERGENCY" className="text-slate-200">Akut</SelectItem>
            <SelectItem value="URGENT" className="text-slate-200">Brådskande</SelectItem>
            <SelectItem value="MODERATE" className="text-slate-200">Måttlig</SelectItem>
            <SelectItem value="LOW" className="text-slate-200">Låg</SelectItem>
          </SelectContent>
        </Select>
      </div>

      <p className="mb-4 text-sm text-slate-400">
        Visar {filteredReports.length} av {total} rapporter
      </p>
      ...
      {loading ? (
        ...
        <Card className="border-white/10 bg-slate-900/50">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-lg font-semibold text-white">Inga rapporter hittades</p>
              ...
              <Card className="border-white/10 bg-slate-900/50 transition-colors hover:border-red-500/30">
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-400">
                        ...
                        {report.side ? ` · ${formatEnum(report.side)}` : ''}
                      </p>
                    </div>
                ...
                <CardContent className="space-y-3">
                  <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-slate-400">
                    ...
                    <span>{mechanismLabels[report.mechanism] || report.mechanism}</span>
                    <span>Smärta {report.initialSeverity}/10</span>
                    <span>Rapporterad av {report.reporter.name || report.reporter.role}</span>
                  </div>
                  {report.description && (
                  ...
                  {report.injury && (
                    <Badge variant="outline" className="border-blue-500/30 bg-blue-500/10 text-blue-300">
                      Kopplad till skada: {report.injury.injuryType || 'bedömning'}
                    </Badge>
                  )}
                </CardContent>
              </Card>
