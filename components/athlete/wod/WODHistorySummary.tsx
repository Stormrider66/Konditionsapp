...
export function WODHistorySummary({ recentWods, stats, basePath = '' }: WODHistorySummaryProps) {
  const t = useTranslations('components.wodHistorySummary')
      ...
      <GlassCardContent className="pt-0">
        ...
        <div className="flex gap-4 mb-3 text-xs">
          <div className="flex items-center gap-1 text-muted-foreground">
            ...
            <span>{t('stats.thisWeek', { count: stats.thisWeek })}</span>
          </div>
          <div className="flex items-center gap-1 text-muted-foreground">
            <CheckCircle2 className="h-3 w-3 text-blue-500" />
            <span>{t('stats.total', { count: stats.totalCompleted })}</span>
          </div>
          <div className="flex items-center gap-1 text-muted-foreground">
                ...
                <div className="flex items-center justify-between p-2 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-xs text-muted-foreground">
                      {format(new Date(wod.completedAt || wod.createdAt), 'd MMM', { locale: dateLocale })}
                    </span>
