...
import { ChevronDown, ChevronUp, CheckCircle2, MapPin, Timer, Moon, Calendar } from 'lucide-react'
    ...
    return (
      <Card>
        <CardContent className="text-center py-12">
          <p className="text-muted-foreground">{t('emptyWeeks')}</p>
        </CardContent>
      </Card>
    )
  }
...
function WeekCard({
  ...
  const phaseLabels: Record<string, string> = {
    ...
    PEAK: t('phase.peak'),
    TAPER: t('phase.taper'),
    RECOVERY: t('phase.recovery'),
    TRANSITION: t('phase.transition'),
  }
          ...
          <div className={cn(
            ...
            <div className="flex items-center gap-6">
              <div className={cn(
                ...
                <span className="text-[9px] font-black uppercase tracking-tighter mb-0.5">{t('week')}</span>
              ...
              <div>
                <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                  <h3 className={cn("text-xl font-black tracking-tight uppercase italic transition-colors", isGlass ? "text-slate-900 dark:text-white" : "")}>
                    {t('phaseLabel')}: <span className="text-blue-600 dark:text-blue-500 transition-colors">{phaseLabels[week.phase] || week.phase}</span>
                  </h3>
                  {isCurrent && (
                ...
                <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 transition-colors">
                  ...
                  <span>
                    {format(weekStartDate, 'd MMM', { locale: dateLocale })} — {format(weekEndDate, 'd MMM yyyy', { locale: dateLocale })}
                  </span>
                </div>
              </div>
            </div>
...
function DayCard({ day, date, isGlass = false, basePath = '' }: DayCardProps) {
  ...
  const intensityLabels: Record<string, string> = {
    RECOVERY: t('intensity.recovery'),
    EASY: t('intensity.easy'),
    MODERATE: t('intensity.moderate'),
    THRESHOLD: t('intensity.threshold'),
    INTERVAL: t('intensity.interval'),
    MAX: t('intensity.max'),
  }
    ...
    return (
      <div className="relative flex items-center gap-6 group">
        ...
        <div className="flex-1 py-4 border-b border-slate-200 dark:border-white/5 opacity-40 italic flex items-center gap-2 transition-colors">
          ...
          <span className="text-xs font-black uppercase tracking-widest text-slate-500">{t('activeRest')}</span>
        </div>
      </div>
    )
  }
