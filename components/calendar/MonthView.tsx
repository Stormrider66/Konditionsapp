 ...
 *
 * Displays a month grid with event indicators on each day
 */
    ...
    for (const item of items) {
      ...
      if (item.endDate && item.type === 'CALENDAR_EVENT') {
        ...
        while (current <= endDate) {
          ...
          // Add reference but mark as continuation
...
function DayCell({ day, isSelected, onClick, onItemClick }: DayCellProps) {
  ...
  return (
    <button
      className={cn(
        'min-h-[80px] md:min-h-[100px] p-1 md:p-2 bg-card text-left transition-colors hover:bg-accent focus:outline-none focus:ring-2 focus:ring-primary focus:ring-inset',
    ...
    >
      ...
      {/* First item preview on larger screens */}
      {day.items.length > 0 && (
