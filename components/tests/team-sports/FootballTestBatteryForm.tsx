...
/**
 * Football Test Battery Form
 *
 * Standard football physical test battery:
 * - Yo-Yo IR1 (Endurance)
 * - 10m Sprint (Acceleration)
 * - 30m Sprint (Max speed)
 * - CMJ (Explosive power)
 */
...
const createFootballTestSchema = (locale: AppLocale) => z.object({
...
type FootballTestFormData = z.infer<ReturnType<typeof createFootballTestSchema>>
...
interface FootballTestBatteryFormProps {
  clients: Client[]
...
export function FootballTestBatteryForm({ clients, onTestSaved }: FootballTestBatteryFormProps) {
  ...
  const form = useForm<FootballTestFormData>({
    resolver: zodResolver(createFootballTestSchema(locale)),
    defaultValues: {
  ...
  const benchmarks = POSITION_BENCHMARKS[position] || POSITION_BENCHMARKS.midfielder
  ...
  async function handleSubmit(data: FootballTestFormData) {
    setSubmitting(true)
    setResults([])
    setError(null)
    ...
    try {
      ...
      if (data.sprint10m) {
        const sprint10Response = await fetch('/api/sport-tests', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            clientId: data.clientId,
        ...
        if (sprint10Response.ok) {
          const result = await sprint10Response.json()
      ...
      console.error('Failed to save football tests:', err)
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('Fotboll - Testbatteri', 'Football - Test battery')}
          </CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t('Fysiska tester för fotbollsspelare: Yo-Yo IR1, Sprint (10m/30m), CMJ', 'Physical tests for football players: Yo-Yo IR1, Sprint (10m/30m), CMJ')}
              </CardDescription>
            </CardHeader>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-4">
              ...
              <TabsTrigger value="sprint10" className="flex items-center gap-1">
