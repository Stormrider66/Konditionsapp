...
import { TestCalculations, TestType } from '@/types'
...
export function SummaryBox({
  ...
  const t = useTranslations('components.summaryBox')
  ...
  const economyEval = bestEconomy
    ? bestEconomy < 200 ? t('economy.excellent') :
      bestEconomy < 210 ? t('economy.veryGood') :
      bestEconomy < 220 ? t('economy.good') :
      bestEconomy < 240 ? t('economy.fair') : t('economy.needsImprovement')
    : null
    ...
    <section className="mt-6 border-b pb-6 print:break-inside-avoid" data-pdf-section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`p-4 rounded-lg border ${
          ...
          {calculations.vo2max && calculations.vo2max > 0 ? (
              ...
              {vo2maxInterpretation && (
                <div className="flex items-center mt-1">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    ['Superior', 'Överlägsen'].includes(vo2maxInterpretation.classification) ? 'bg-green-200 text-green-800' :
                    ['Excellent', 'Utmärkt'].includes(vo2maxInterpretation.classification) ? 'bg-green-100 text-green-700' :
                    ['Good', 'God'].includes(vo2maxInterpretation.classification) ? 'bg-blue-100 text-blue-700' :
            ...
            <p className="text-sm text-gray-500 italic mt-1">{t('notMeasured')}</p>
          )}
        </div>

        {/* Anaerobic Threshold */}
        <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
          <p className="text-gray-600 text-sm">{t('anaerobicThreshold')}</p>
            ...
            <>
              ...
              <div className="text-sm text-gray-600 mt-1">
                ...
                <span className={`font-medium ${
                  ...
                  {t('percentOfMax', { percent: calculations.anaerobicThreshold.percentOfMax })}
                </span>
              </div>
            </>
          ) : (
            <p className="text-sm text-gray-500 italic mt-1">{t('notCalculated')}</p>
          )}
        </div>
        ...
        {testType === 'RUNNING' ? (
          <div className={`p-4 rounded-lg border ${
            ...
            <p className="text-gray-600 text-sm">{t('runningEconomy')}</p>
            {bestEconomy ? (
              <>
                ...
                <div className="flex items-center mt-1">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    economyEval === t('economy.excellent') ? 'bg-green-200 text-green-800' :
                    economyEval === t('economy.veryGood') ? 'bg-green-100 text-green-700' :
                    economyEval === t('economy.good') ? 'bg-blue-100 text-blue-700' :
                    'bg-gray-100 text-gray-700'
                  }`}>
                    {economyEval}
                  </span>
              ...
              <p className="text-sm text-gray-500 italic mt-1">{t('requiresVo2')}</p>
            )}
          </div>
        ) : testType === 'CYCLING' && calculations.cyclingData ? (
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <p className="text-gray-600 text-sm">{t('powerWeight')}</p>
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
            <p className="text-gray-600 text-sm">{t('maxLactate')}</p>
      ...
      <div className="mt-4 p-4 bg-gradient-to-r from-slate-50 to-slate-100 rounded-lg border border-slate-200">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div>
            <span className="text-sm text-gray-500">{t('athleteProfile')}</span>
