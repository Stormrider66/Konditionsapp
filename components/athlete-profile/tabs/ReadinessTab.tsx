...
export function ReadinessTab({ data, viewMode: _viewMode, variant = 'default' }: ReadinessTabProps) {
  ...
  const chartData = [...dailyCheckIns]
    ...
    .map((checkIn) => ({
      date: format(new Date(checkIn.date), 'EEE', { locale: dateLocale }),
      ...
      <CardWrapper>
        <CardContent className="py-20 text-center">
          ...
          <h3 className={cn("text-xl font-black uppercase italic tracking-tight mb-2", isGlass ? "text-slate-900 dark:text-white" : "text-gray-900")}>
            {t('Ingen beredskapsdata', 'No readiness data')}
          </h3>
          <p className={cn("font-medium max-w-sm mx-auto", isGlass ? "text-slate-500" : "text-gray-500")}>
            {t('Aktivera daglig incheckning för att spåra träningsberedskap.', 'Enable daily check-ins to track training readiness.')}
          </p>
        </CardContent>
      </CardWrapper>
    )
  ...
  return (
    <div className="space-y-8">
          ...
          <CardContent className="p-8">
            <div className="flex flex-col lg:flex-row items-center gap-10">
              ...
              <div className="text-center lg:pr-10 lg:border-r border-white/5">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">{t('Dagens beredskap', "Today's readiness")}</p>
                <div
                  className={cn(
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-600">{t('skala 1-10', 'scale 1-10')}</p>
                {latestCheckIn.readinessDecision && (
                  <Badge
                    className={cn(
                      "mt-4 font-black uppercase tracking-widest text-[8px] h-5 rounded-md border-0",
                      latestCheckIn.readinessDecision === 'REST' ? 'bg-red-500 text-white' : 'bg-emerald-500 text-white'
                    )}
                  >
                    {getDecisionLabel(latestCheckIn.readinessDecision, locale)}
                  </Badge>
                )}
              </div>
              ...
              <div className="flex items-baseline gap-2 mb-4">
                ...
                {latestMetrics?.hrvStatus && (
                  <Badge className={cn(
                    ...
                    {getHrvStatusLabel(latestMetrics.hrvStatus, locale)}
                  </Badge>
                )}
              </div>
              ...
              <div className="flex items-baseline gap-2 mb-4">
                ...
                {latestMetrics?.restingHRStatus && (
                  <Badge className={cn(
                    ...
                    {getRhrStatusLabel(latestMetrics.restingHRStatus, locale)}
                  </Badge>
                )}
              </div>
      ...
      {chartData.length >= 7 && (
        <CardWrapper>
          <CardHeader>
            <CardTitle className={cn("text-xl font-black uppercase italic tracking-tight", isGlass ? "text-white" : "")}>{t('Beredskapstrend', 'Readiness trend')}</CardTitle>
            <CardDescription className={cn("font-black uppercase tracking-widest text-[10px]", isGlass ? "text-slate-500" : "")}>{t('SENASTE 14 DAGARNA', 'LAST 14 DAYS')}</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardWrapper>
            <CardHeader>
              <CardTitle className={cn("text-xl font-black uppercase italic tracking-tight flex items-center gap-2", isGlass ? "text-white" : "")}>
                ...
                {t('Menscykel', 'Menstrual cycle')}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className={cn("p-4 rounded-2xl", isGlass ? "bg-pink-500/5 border border-pink-500/10" : "bg-pink-50")}>
                  <p className="text-[10px] font-black uppercase tracking-widest text-pink-500/70 mb-1">{t('Nuvarande fas', 'Current phase')}</p>
                  <p className={cn("text-lg font-black uppercase italic", isGlass ? "text-pink-400" : "text-pink-800")}>{getCyclePhaseLabel(latestCycle.currentPhase || '', locale)}</p>
                </div>
                <div className={cn("p-4 rounded-2xl", isGlass ? "bg-white/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5" : "bg-gray-50")}>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">{t('Cykeldag', 'Cycle day')}</p>
                  <p className={cn("text-lg font-black italic", isGlass ? "text-slate-900 dark:text-white" : "text-gray-900")}>{t('Dag', 'Day')} {calculateCycleDay(latestCycle.startDate)}</p>
                </div>
              </div>
            </CardContent>
          </CardWrapper>
      ...
      <CardWrapper>
        <CardHeader>
          ...
          <CardDescription className={cn("font-black uppercase tracking-widest text-[10px]", isGlass ? "text-slate-500" : "")}>{t('SENASTE 7 DAGARNA', 'LAST 7 DAYS')}</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className={cn("font-black uppercase italic text-xs tracking-tight", isGlass ? "text-slate-900 dark:text-white" : "text-gray-900")}>
                      {format(new Date(checkIn.date), 'EEEE d MMMM', { locale: dateLocale })}
                    </p>
                    {checkIn.readinessDecision && (
                      <Badge className={cn("font-black uppercase tracking-widest text-[8px] h-4 rounded-md border-0 mt-1", isGlass ? "bg-white/50 dark:bg-white/5 text-slate-500" : "bg-white border")}>
                        {getDecisionLabel(checkIn.readinessDecision, locale)}
                      </Badge>
                    )}
                  </div>
                </div>
...
function getDecisionLabel(decision: string, locale: 'en' | 'sv'): string {
  const labels: Record<'en' | 'sv', Record<string, string>> = {
    en: {
      PROCEED: 'Normal training',
      REDUCE: 'Reduce',
      EASY: 'Easy training',
      REST: 'Rest',
    },
  ...
  return labels[locale][decision] || decision
}

function getHrvStatusLabel(status: string, locale: 'en' | 'sv'): string {
  const labels: Record<'en' | 'sv', Record<string, string>> = {
    en: {
      OPTIMAL: 'Optimal',
      ELEVATED: 'Elevated',
...
function getRhrStatusLabel(status: string, locale: 'en' | 'sv'): string {
  const labels: Record<'en' | 'sv', Record<string, string>> = {
    en: {
      NORMAL: 'Normal',
      ELEVATED: 'Elevated',
...
function getCyclePhaseLabel(phase: string, locale: 'en' | 'sv'): string {
  const labels: Record<'en' | 'sv', Record<string, string>> = {
    en: {
      MENSTRUAL: 'Menstrual',
