...
export async function getMuscleGroupData(
  ...
  // Get all set logs for this athlete in the date range, with exercise info
  ...
  // Assign each set log to its period bucket
  ...
  // Build summary across all periods
  const summary: Record<CanonicalMuscleGroup, { volume: number; sets: number }> = {} as any
  for (const g of CANONICAL_MUSCLE_GROUPS) {
