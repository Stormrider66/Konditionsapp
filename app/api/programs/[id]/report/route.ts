// app/api/programs/[id]/report/route.ts
//
// Compiled program report consumed by ProgramReportViewer.
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    logError('Error building program report', error)
    return NextResponse.json(
      { error: t(locale, 'Failed to build report', 'Kunde inte skapa rapporten') },
      { status: 500 }
    )
  }
}
