...
export async function GET(request: NextRequest) {
  try {
    ...
    if (clientId) {
      whereClause.trainingDay.trainingWeek.program.clientId = clientId
    }
    ...
    const modifications = workouts.map(workout => {
      const client = workout.trainingDay.trainingWeek.program.client

      // Determine original type from notes (if available)
      let originalType = 'UNKNOWN'
      if (workout.notes) {
        const match = workout.notes.match(/Original:\s*(\w+)/)
        if (match) {
          originalType = match[1]
        }
      }

      // Determine modification action from status and notes
      let action: 'CANCEL' | 'REDUCE_INTENSITY' | 'REDUCE_VOLUME' | 'CONVERT_TO_CROSS_TRAINING'
      if (workout.status === 'CANCELLED') {
        action = 'CANCEL'
      } else if (workout.type === 'CROSS_TRAINING') {
        action = 'CONVERT_TO_CROSS_TRAINING'
      } else if (workout.intensityType === 'EASY') {
        action = 'REDUCE_INTENSITY'
      } else {
        action = 'REDUCE_VOLUME'
      }

      // Extract reasoning from notes
      const reasoning =
        workout.notes || 'Automatic modification due to injury/fatigue detection'

      // Check if this was auto-generated (look for keywords in notes)
      const autoGenerated =
        workout.notes?.includes('pain level') ||
        workout.notes?.includes('Complete rest required') ||
        workout.notes?.includes('Convert to cross-training') ||
        workout.notes?.includes('50% volume reduction') ||
        false
      ...
      return {
        ...
        workoutDate: workout.trainingDay.date,
        originalType,
    ...
    console.error('Error fetching workout modifications:', error)
    return NextResponse.json(
      {
