...
function makeRequest(method = 'GET') {
  return new NextRequest('http://localhost/api/clients/client-1/paces', {
    method,
    ...(method === 'POST'
      ? {
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ preferredSource: 'LACTATE_RATIO', manualLT1Stage: 2 }),
        }
      : {}),
  })
}
...
describe('/api/clients/[id]/paces', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockSupabaseGetUser.mockResolvedValue({ data: { user: { id: 'coach-1' } } })
    ...
    mockPrisma.athleteProfile.update.mockResolvedValue({})
    mockPrisma.athleteProfile.create.mockResolvedValue({})
    mockSelectOptimalPaces.mockReturnValue(mockPaceSelection())
  })

  it('GET ignores tests that are still waiting for quality review', async () => {
  ...
  it('POST recalculation also ignores tests still waiting for quality review', async () => {
