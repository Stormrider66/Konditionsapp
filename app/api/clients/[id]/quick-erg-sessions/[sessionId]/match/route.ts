...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json({ success: false, error: t(locale, 'Session is already matched', 'Passet är redan matchat') }, { status: 409 })
    }
    ...
    if (assignment.status === 'SKIPPED') {
      return NextResponse.json({ success: false, error: t(locale, 'Skipped sessions cannot be matched', 'Överhoppade pass kan inte matchas') }, { status: 409 })
    }

    if (assignment.status === 'COMPLETED' || assignment.completedAt) {
      return NextResponse.json({ success: false, error: t(locale, 'Planned session is already completed', 'Planerat pass är redan klart') }, { status: 409 })
    }

    if (assignment.assignedDate > dayDate(new Date())) {
      return NextResponse.json({ success: false, error: t(locale, 'Future sessions cannot be completed yet', 'Framtida pass kan inte markeras klara än') }, { status: 409 })
    }
    ...
    logger.info('Coach matched quick erg session to planned cardio assignment', {
      coachId: user.id,
      clientId,
    ...
    logger.error('Failed to match quick erg session as coach', {}, error)
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    if (!match) {
      return NextResponse.json({ success: false, error: t(locale, 'Session is not matched', 'Passet är inte matchat') }, { status: 409 })
    }
    ...
    logger.info('Coach removed quick erg planned cardio match', {
      coachId: user.id,
      clientId,
      quickErgSessionId: session.id,
    ...
    logger.error('Failed to remove quick erg planned match as coach', {}, error)
    return NextResponse.json(
