...
export async function GET(
  ...
  try {
    ...
    if (!screening) {
      return NextResponse.json({ error: t(locale, 'Screening not found', 'Screeningen hittades inte') }, { status: 404 })
    }
    ...
    return NextResponse.json(
      { error: t(locale, 'Failed to fetch screening', 'Kunde inte hämta screeningen') },
      { status: 500 }
    )
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json(
      { error: t(locale, 'Failed to update screening', 'Kunde inte uppdatera screeningen') },
      { status: 500 }
    )
  }
}
