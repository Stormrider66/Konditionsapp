...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (duration > MAX_DURATION) {
      return NextResponse.json(
        { error: t(locale, `Recording too long. Maximum duration: ${MAX_DURATION / 60} minutes`, `Inspelningen är för lång. Maxlängd: ${MAX_DURATION / 60} minuter`) },
        { status: 400 }
      )
    }
    ...
    if (!googleKey) {
      return NextResponse.json(
        { error: t(locale, 'Google API key not configured. Add API key in settings.', 'Google API-nyckel är inte konfigurerad. Lägg till API-nyckel i inställningarna.') },
        { status: 400 }
      )
    }
    ...
    if (uploadError) {
      ...
      const errorMessage = uploadError.message?.includes('bucket')
        ? t(locale, 'Storage bucket "voice-workouts" not found. Please create it in Supabase.', 'Storage-bucket "voice-workouts" hittades inte. Skapa den i Supabase.')
        : uploadError.message?.includes('policy')
        ? t(locale, 'Storage permission denied. Check bucket policies.', 'Storage-behörighet nekades. Kontrollera bucket-policies.')
        : t(locale, `Failed to upload audio file: ${uploadError.message}`, `Kunde inte ladda upp ljudfilen: ${uploadError.message}`)
    ...
    try {
      ...
      await prisma.voiceWorkoutSession.update({
        ...
        data: {
          status: 'FAILED',
          errorMessage: aiError instanceof Error ? aiError.message : t(locale, 'AI processing failed', 'AI-bearbetning misslyckades'),
        },
      })
    ...
    return NextResponse.json({ error: t(locale, 'Failed to process voice workout', 'Kunde inte bearbeta röstpasset') }, { status: 500 })
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: t(locale, 'Failed to fetch voice workout sessions', 'Kunde inte hämta röstpass') }, { status: 500 })
  }
}
