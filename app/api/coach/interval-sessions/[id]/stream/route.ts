/**
 * Interval Session Stream API (SSE)
 *
 * GET - Server-Sent Events stream for real-time session updates
 */

import { NextRequest } from 'next/server'
import { getSessionStreamData } from '@/lib/interval-session/session-service'
...
export async function GET(req: NextRequest, context: RouteContext) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Poll every 1 second (faster than LiveHR for timing precision)
