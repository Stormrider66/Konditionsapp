...
**Status:** ✅ **All phases (1–5) shipped to `main` 2026-05-30 — redesign complete.** The Idag tab renders the phase + attention strips over an interactive two-pane cockpit (live schedule timeline | compact roster rail) plus the auto-hiding setup banner. Cross-pane linking is live: click a session → its players light up in the rail (rest dimmed); click a player → their sessions light up in the schedule (rest dimmed), or a quick-assign CTA appears if they have none; a position filter narrows both panes at once; the active session pulses "now". The Uppföljning adherence discrepancy is resolved (single canonical source + missed→names reframe). Follow-ups worth tracking are listed under *Open questions / later*.
...
4. **Cross-pane interactions**, position filter across both panes, live "now" highlighting. ✅ **Shipped 2026-05-30.** A new client shell `TeamCockpit.tsx` lifts the day-event fetch out of the schedule pane and owns the shared interaction state (selected session/player + position filter); the schedule pane and rail became presentational. Click a session → highlight its participants in the rail + dim the rest (participants read from `assignmentSummary.athletes[].athleteId`, which equals `Client.id`); click a player → highlight their sessions + dim the rest, or show a quick-assign CTA (→ team calendar) when they have none today; selection toggles off on re-click and clears on day-nav. Position filter narrows the rail and dims schedule cards whose *known* roster lacks that position (sessions with unknown rosters stay visible — never hide on unknown). Active sessions get an emerald left-border + a pulsing "now" dot (status captured at load/nav, not a ticking timer). Verified with typecheck + eslint. Schedule cards became keyboard-accessible `role=button` divs (an anchor-in-button would've been invalid HTML). **Follow-up (resolved 2026-05-30):** the rail's "pass idag" column is now day-aware — it derives each member's active/completed coverage from the *viewed day's* events (the widened `assignmentSummary.athletes[].status`), so it tracks the schedule's day-nav with zero extra requests. The status dot deliberately stays current-state (readiness/medical isn't day-specific), and the attention strip stays a "today" triage line. Verified additionally with a full `npm run build`.
5. **Fix + reframe the adherence analytics** in Uppföljning (resolve the discrepancy). ✅ **Shipped 2026-05-30.** Root cause: two independent computations disagreed — the four server-rendered stat cards (N+1 per-member counts; strength/cardio/hybrid broadcasts only — *no agility, no intervals*; completion = `status==='COMPLETED'` only; hard-coded 30d) vs. the `TeamWorkoutMonitor` panel (all four broadcast types **+ interval sessions**; completion = `status || completedAt || resultId`; 7/30/90 window; full per-player breakdown). **Fix = make the monitor the single source:** deleted the cards and their divergent query from `uppfoljning/page.tsx`, so a contradiction is now structurally impossible. **Reframe:** the headline "Missade" stat is now an actionable button and, whenever `missed > 0`, an amber banner explains that a low completion rate is usually *unlogged* rather than skipped training and offers **"Visa vilka spelare"** — which expands the panel, switches to the Players tab and applies the missed filter, landing the coach on names instead of a vanity %. Verified with typecheck + eslint.
